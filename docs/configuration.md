...
- `adapter.version` (string, required): Adapter version expected by the binary.
...
- `auth.tls_config.ca_file` (string): CA certificate path.
- `auth.tls_config.cert_file` (string): Client certificate path.
- `auth.tls_config.key_file` (string): Client key path.
